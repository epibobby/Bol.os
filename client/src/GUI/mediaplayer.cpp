#include <QtGui>
 ...
 {
     _menuOn = false;
     _menuTimer = new QTimer();
     menu = new myWindow(this); //Ajout de notre classe myWindow
     m_action.setCheckable(true);
     m_action.setChecked(false);
     m_action.setShortcut(QKeySequence( Qt::AltModifier + Qt::Key_Return));
     m_action.setShortcutContext(Qt::WindowShortcut);
     connect(&m_action, SIGNAL(toggled(bool)), SLOT(setFullScreen(bool)));
     _menuTimer->connect(_menuTimer, SIGNAL(timeout()),this, SLOT(menuOff()));
 ...
 {
    // test->move((width()/2) -(test->width()/2), ((height()*90)/100));
     Phonon::VideoWidget::mouseDoubleClickEvent(e);
     setFullScreen(!isFullScreen());
 }

 void MediaVideoWidget::mouseMoveEvent(QMouseEvent *)
 {
     std::cout << "Height MediaVideo=" << this->height() << std::endl;
     menu->setSize(width(), height());
     //menu->paintGL();
 }
 void MediaVideoWidget::mousePressEvent(QMouseEvent *e)
 {
     if (e->x() >= ((width()/2) - (menu->width()/2)) + 100 && e->y() >= ((height()*92)/100) && e->y() <= ((height()*96)/100) && e->x() <= ((width()/2) + (menu->width()/2)) - 110) {
            std::cout << "PMouse cliked" << std::endl;
            m_player->playPause();
     }
     else if (e->x() >= ((width()/2) - (menu->width()/2)) && e->y() >= ((height()*92)/100) && e->y() <= ((height()*96)/100) && e->x() <= ((width()/2) + (menu->width()/2)) - 180) {
            std::cout << "PMouse cliked" << std::endl;
            m_player->rewind();
     }
     else if (e->x() >= ((width()/2) - (menu->width()/2)) + 180 && e->y() >= ((height()*92)/100) && e->y() <= ((height()*96)/100) && e->x() <= ((width()/2) + (menu->width()/2)) - 50) {
            std::cout << "PMouse cliked" << std::endl;
            m_player->forward();
     }
     menu->paintGL();
 ...
 void MediaVideoWidget::menuOn()
 {
     if (_menuOn == false) {
         _menuOn = true;
         menu->show();   //Exécution de notre fenêtre de rendu OpenGL // MenuOn
         _menuTimer->start(5000);
     }
 }

 void MediaVideoWidget::menuOff()
 {
     _menuOn = false;
     menu->hide();
 }
 ...
 {
     setWindowTitle(tr("LMC Media Player"));
     ...
     AutoS = new AutoShutdown;
     AutoS->setWindowTitle(tr("LMC AutoShutdown"));
     AutoS->setMinimumSize(182, 163);
     AutoS->setMaximumSize(182, 163);
     ...
     QIcon shutdownIcon;
     QImage img;
     img.load("images/ShutDown.png");
     shutdownIcon.addPixmap(QPixmap::fromImage(img));

     //shutdownIcon = style()->standardIcon(QStyle::SP_MediaPause);
 ...
 {
    m_videoWidget->menu->loadTextureMenu();
